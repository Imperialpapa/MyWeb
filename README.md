...
| `supabase/functions/ai/index.ts` | **서비스 안 AI 에이전트** (Supabase Edge Function). 올릴 때 분류 제안, 운영자용 재분류·분야 구조 제안·자료 채우기. 링크 생존·중복·위험 명령 검증도 여기서. Claude API 키는 여기서만 쓰임 |
...
사이트 안에 **분류 담당 AI 에이전트**가 들어 있습니다. 네 가지 일을 합니다.

| 어디서 | 누가 | 무엇을 |
|---|---|---|
| 새 항목 창 → **✦ AI 분류 제안** | 로그인한 사람 | 제목·주소를 보고 분야·하위분야·태그를 채움. 제목이 비었으면 제목도, 링크 설명이 비었으면 한 줄 설명도 제안 |
| 분야 옆 **✦ AI 정리** → 항목 다시 분류 | 운영자 | 분야가 빈 항목·오래 확인 안 한 것·보이는 목록·전체를 30개씩 검토해 바꿀 것만 표로 제안. 링크가 살아 있는지도 함께 확인. 체크한 것만 적용 |
| **✦ AI 정리** → 분야 구조 개편안 | 운영자 | 전체 자료를 보고 하위분야를 더하고 빼는 안을 제안. 고쳐서 저장 |
| **✦ AI 정리** → 분야에 자료 채우기 | 운영자 | 고른 분야(또는 새로 만들 분야)에 넣을 자료를 5~10개 제안. 체크한 것만 추가 |
...
**자료 채우기는 AI 말을 그대로 믿지 않습니다.** 서버가 제안된 링크를 실제로 한 번 열어 보고(살아 있음 · 없음 · 확인 불가),
이미 있는 자료와 주소·제목이 겹치는지 보고, 되돌릴 수 없는 명령(디스크 포맷, 강제 덮어쓰기 등)이 섞이면 아예 고를 수 없게 막습니다.
겹치거나 링크가 죽은 항목은 체크가 꺼진 채로 나옵니다. 그래도 **메모와 코드의 내용이 맞는지는 기계가 판단할 수 없으니 눈으로 읽고 고르세요.**

1. https://console.anthropic.com 에서 API 키를 만듭니다 (`sk-ant-...`). 결제 수단 등록이 필요합니다. 비용은 항목 하나 분류에 1원 안팎, 50개 재분류에 50원 안팎입니다.
   비용 방어선으로 한 사람이 하루에 쓸 수 있는 **사용량**을 20(운영자 300)으로 제한해 두었습니다.
   작업마다 무게가 달라서 분류 제안은 1, 분야 구조는 7, 재분류는 한 묶음(30개)에 9, 자료 채우기는 한 번에 15를 씁니다.
   이 무게는 실제 토큰 비용의 비율입니다. 운영자가 하루 한도를 다 써도 대략 7,500원 안쪽입니다.
   바꾸려면 `supabase/functions/ai/index.ts` 의 `DAILY_LIMIT`·`ACTION_COST` 를 고치고 다시 배포하세요.

   **호출마다 토큰 사용량이 로그에 남습니다.** Supabase 대시보드 **Edge Functions → ai → Logs** 에서
   `{"ai":"expand","in":...,"cached":...,"out":...,"ms":...}` 형태로 볼 수 있습니다. 어느 작업이 실제로 비싼지는 이걸로 판단하세요.

   **Anthropic 콘솔에서 지출 한도도 걸어 두세요.** 위 한도는 우리 쪽 방어선이고, 계정 차원의 상한은 별개입니다.
   console.anthropic.com → Settings → Limits 에서 월 지출 상한을 정하면 그 위로는 청구되지 않습니다.
...
> 분야 이름은 마음대로 바꿔도 됩니다. 자료·담당자 지정·권한은 이름이 아니라 분야마다 붙은 **id** 로 이어져 있어 개명해도 그대로 따라옵니다.
> 반대로 id 는 바꾸면 안 됩니다. 화면에는 나오지 않으니 신경 쓸 일이 없지만, seed 파일이나 `schema.sql` 을 손으로 고칠 때는 id 를 건드리지 마세요.
...
| 신고 검토함 전체, 분야 편집, 주 관리자 지정, AI 정리(재분류·분야 구조·자료 채우기) | | | O |
